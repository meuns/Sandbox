...
WORLD_INT_LINE_COUNT = 128
WORLD_INT_X_JITTER = 0.0
WORLD_INT_Y_JITTER = 0.0

INTX_LINE_COUNT = WORLD_INT_LINE_COUNT // 2
INTX_POINT_COUNT = INTX_LINE_COUNT + 1
INT0_X = [WORLD_INT_X_JITTER * random() + -1.0 + (i / INTX_LINE_COUNT) * 2.0 for i in range(INTX_POINT_COUNT)]
INT0_Y = [WORLD_INT_Y_JITTER * random() + +0.2 for i in range(INTX_POINT_COUNT)]
INT1_X = [WORLD_INT_X_JITTER * random() + -1.0 + (i / INTX_LINE_COUNT) * 2.0 for i in range(INTX_POINT_COUNT)]
INT1_Y = [WORLD_INT_Y_JITTER * random() + -0.2 for i in range(INTX_POINT_COUNT)]
INT_X = [INT0_X[44:47]] #, INT1_X]
INT_Y = [INT0_Y[44:47]] #, INT1_Y]
WORLD_LINE_COUNT = len(INT_X[0]) - 1 #+ len(INT_X[1]) - 1
